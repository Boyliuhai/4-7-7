...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	//GetMenu()->GetSubMenu(5) ->CheckMenuItem(0, MF_BYPOSITION | MF_CHECKED);
	...
	//SetMenu(NULL);取消菜单
	//
	/*CMenu menu;建立菜单。。。。。。。。。。。。定义为全局变量，CMainFrame
	menu.LoadMenu(IDR_MAINFRAME);
	SetMenu(&menu);*/
	//menu.Detch();...消除局部变量的尴尬 
	...
	//GetMenu()->GetSubMenu(0)->InsertMenu(ID_FILE_OPOEN,MF_BYCOMMAND | MF_STRING,115,"微信"); 
	//GetMenu()->DeleteMenu(1,MF_BYPOSITION); 
	//GetMenu()->GetSubMenu(0)->DeleteMenu(2,MF_BYPOSITION);
	
	//1先在resource中添加，
	//2在头文件和源文件中学消息原型和消息映射
	//3
	...
	//SetWindowLong(m_hWnd, GWL_STYLE, WS_OVERLAPPEDWINDOW);没有标题
	//SetWindowLong(m_hWnd, GWL_STYLE, GetWindowLong(m_hWnd, GWL_STYLE)& ~WS_MAXIMIZEBOX);最大化失效
	
	//SetClassLong(m_hWnd, GCL_HICON, (LONG)LoadIcon(NULL, IDI_ERROR));
	//SetClassLong是一个函数，该函数的功能：它会替换存储空间中指定偏移量处的32位长整型值，
	//或替换指定窗口所属类的WNDCLASSEX结构(应该是替换这个结构体中的值，并没有把结构体给换了)。
	//三种方式
	m_hIcons[0] = LoadIcon(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDI_ICON3));
	m_hIcons[1] = LoadIcon(theApp.m_hInstance, MAKEINTRESOURCE(IDI_ICON4));
	m_hIcons[2]= LoadIcon(AfxGetApp()->m_hInstance, MAKEINTRESOURCE(IDI_ICON5));
...
BOOL CMainFrame::PreCreateWindow(CREATESTRUCT& cs)
{
	...
	//wndcls.lpfnWndProc = ::DefWindowProc;//调用的是全局API 函数
...
void CMainFrame::OnUpdateEditCut(CCmdUI *pCmdUI)//剪切
{
	//if(2==pCmdUI->m_nIndex)-----索引
	//if(ID_EDIT_CUT==pCmdUI->m_nID)直接
	pCmdUI->Enable();
